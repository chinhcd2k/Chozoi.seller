...
import {CREATE_PRODUCT_CTRL} from "./CreateProductControl";
...
export default class CreateProduct extends React.Component<ICreateProductProps, ICreateProductState> {
  ...
  public store = new CreateProductStore();
  ...
  constructor(props: ICreateProductProps) {
    super(props);
    CREATE_PRODUCT_CTRL.view = this;
    CREATE_PRODUCT_CTRL.store = this.store;
  ...
  protected handlerOnChangeClassifierOnClick() {
    ...
    CREATE_PRODUCT_CTRL.initTableVariantData();
  }
                  ...
                  {this.store.getListClassifier.map((item, index) => <tr key={index}>
                        ...
                        {item.values.length > 0 && <ul className="option-values">
                          {item.values.map((name: string, index1: number) => <li key={index1}>
                            ...
                            <i className="fal fa-times"
                               onClick={() => CREATE_PRODUCT_CTRL.removeValueClassifier(item, index, index1)}/>
                          </li>)}
                        </ul>}
                    ...
                    <td>
                      <button type="button" className="btn btn-default"
                              onClick={() => CREATE_PRODUCT_CTRL.removeClassifier(index)}>
        ...
        {/*Ap dung*/}
          <div className="mt-3 w-100 d-flex align-items-center apply-value">
              <input type="text"
                     ...
                     onChange={e => CREATE_PRODUCT_CTRL.handlerOnChangeDefaultValueVariant(e, "price")}
              />
              <input type="text"
                     className="ml-3 form-control"
                     placeholder="Giá bán(*)"
                     onChange={e => CREATE_PRODUCT_CTRL.handlerOnChangeDefaultValueVariant(e, "sale_price")}
              />
              <input type="text"
                     className="ml-3 form-control"
                     placeholder="Kho hàng"
                     onChange={e => CREATE_PRODUCT_CTRL.handlerOnChangeDefaultValueVariant(e, "quantity")}
              />
              <input type="text"
                     className="ml-3 form-control"
                     placeholder="SKU phân loại"
                     name="sku"
                     onChange={e => CREATE_PRODUCT_CTRL.handlerOnChangeDefaultValueVariant(e, "sku")}
              />
              <button className="ml-3 btn btn-dark"
                      type="button"
                      onClick={() => CREATE_PRODUCT_CTRL.handlerApplyDefaultValueVariant()}>Áp dụng
              </button>
          </div>
    ...
    return <div className="panel-body">
      ...
      <div className="row">
        <div className="col-xs-6">
          ...
          <FormGroup className="form-group">
            <Input
              ...
              onChange={e => CREATE_PRODUCT_CTRL.handlerOnChangePriceAndStore(e, "price")}
            />
        ...
        <div className="col-xs-6">
          ...
          <FormGroup className="form-group">
            <Input
              onChange={e => CREATE_PRODUCT_CTRL.handlerOnChangePriceAndStore(e, "sale_price")}
              className="form-control"
              placeholder="Giá bán"
              name={"sale_price"}
        ...
        <div className="col-xs-6">
          ...
          <div className="form-group">
            <input className="form-control" type="text"
                   onChange={e => CREATE_PRODUCT_CTRL.handlerOnChangePriceAndStore(e, "sku")}
                   placeholder="Mã sku"/>
          </div>
        </div>
      </div>
        ...
        <div className="col-xs-12 col-lg-6">
          ...
          {this.store.getIsQuantityLimited && <FormGroup className="form-group">
              ...
              <Input
                  ...
                  onChange={e => CREATE_PRODUCT_CTRL.handlerOnChangePriceAndStore(e, "quantity")}
                  validations={[
  ...
  handlerOnCreate(name?: string) {
    ...
    if (CREATE_PRODUCT_CTRL.request_body_data_create_product) {
      CREATE_PRODUCT_CTRL.request_body_data_create_product.name = name || '';
    }
    // Re render
  ...
  async handlerOnFetchData(data: IResponseProduct | IResponseAuction) {
    this.isShowIntro = false;
    CREATE_TEMPLATE_CONTROL.store.defaultName = data.name;
    CREATE_TEMPLATE_CONTROL.setDefaultValueCategories(data.category.id).then(() => {
      CREATE_TEMPLATE_CONTROL.setDefaultValuePropety(data.category.id, data.attributes);
    });
    CREATE_TEMPLATE_CONTROL.store.defaultWeight = data.weight;
    CREATE_TEMPLATE_CONTROL.store.defaultPackageSize = data.packingSize;
    CREATE_TEMPLATE_CONTROL.store.defaultDescPickingOut = data.descriptionPinkingOut;
    CREATE_TEMPLATE_CONTROL.store.listImage = [];
    (data as IResponseProduct).images.map((value, index) => CREATE_TEMPLATE_CONTROL.store.listImage.push({
      sort: index,
      src: value.imageUrl,
      file: null
    }));
    ...
    if (CREATE_PRODUCT_CTRL.request_body_data_create_product) {
      const bodyData = CREATE_PRODUCT_CTRL.request_body_data_create_product;
      bodyData.name = data.name;
      bodyData.category.id = data.category.id;
      bodyData.attributes = [];
      (data as IResponseProduct).attributes.map(value => bodyData.attributes.push({
        id: value.id,
        value_id: value.valueId
      }));
      bodyData.weight = data.weight;
      bodyData.packing_size = data.packingSize;
      bodyData.description_pickingout = data.descriptionPinkingOut || "";
    }
    // Re render
    CREATE_TEMPLATE_CONTROL.store.key = Date.now();
  }
  ...
  render(): React.ReactNode {
    ...
    return <div id="create-product">
      ...
      <div style={{display: this.isShowIntro ? "none" : "block"}}>
        {this.renderWarring}
        <Form className="container" onSubmit={(e: any) => CREATE_PRODUCT_CTRL.handlerOnSubmit(e)}>
          <div className="row">
          ...
          <div className="row mt-3">
            <CreateTemplate
              ...
              EmitInputOnChange={(e, key) => CREATE_PRODUCT_CTRL.handlerOnChangeFormField(e, key)}>
          ...
          <div className="row mt-5">
            <div className="col-xs-12">
              <button className="btn btn-default" type="submit"
                      ...
                      onClick={() => {
                        CREATE_PRODUCT_CTRL.handlerBtnOnSubmit(0);
              ...
              <button className="btn btn-primary mx-3" type="submit"
                      disabled={this.state.disabledSubmit}
                      onClick={() => {
                        CREATE_PRODUCT_CTRL.handlerBtnOnSubmit(1);
                        ga.pushEventGa('Submit_product_s2', 'Click_save_submit');
                      }}>Lưu và Đăng
              </button>
              <button className="btn btn-danger" type="reset"
                      onClick={() => {
                        CREATE_PRODUCT_CTRL.handlerBtnOnSubmit(2);
  ...
  componentDidUpdate(prevProps: Readonly<ICreateProductProps>, prevState: Readonly<ICreateProductState>, snapshot?: any): void {
    if (prevState.applyDefaultValueVariant !== this.state.applyDefaultValueVariant) {
      this.state.applyDefaultValueVariant && CREATE_PRODUCT_CTRL.handlerApplyDefaultValueVariant();
    }
  }
}
